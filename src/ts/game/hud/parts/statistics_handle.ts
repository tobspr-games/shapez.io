...
export class HUDShapeStatisticsHandle {
    ...
    setVisible(visibility: boolean) {
        ...
        this.element.classList.toggle("pinned", this.root.hud.parts.pinnedShapes.isShapePinned(this.definition.getHash()));
        switch (dataSource) {
            case enumAnalyticsDataSource.stored: {
                this.counter.innerText = formatBigNumber(this.root.hubGoals.storedShapes[this.definition.getHash()] || 0);
                break;
            }
