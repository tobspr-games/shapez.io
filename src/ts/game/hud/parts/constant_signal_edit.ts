...
export class HUDConstantSignalEdit extends BaseHUDPart {
    initialize(): any {
        ...
        if (contents) {
            const constantComp: any = contents.components.ConstantSignal;
            if (constantComp) {
     ...
     */
    editConstantSignal(entity: Entity, { deleteOnCancel = true }: {
        ...
        const signal: any = entity.components.ConstantSignal.signal;
        const signalValueInput: any = new FormElementInput({
            id: "signalValue",
            label: fillInLinkIntoTranslation(T.dialogs.editSignal.descShortKey, THIRDPARTY_URLS.shapeViewer),
            placeholder: "",
            defaultValue: signal ? signal.getAsCopyableKey() : "",
            validator: (val: any): any => this.parseSignalCode(entity, val),
        });
        const items: any = [...Object.values(COLOR_ITEM_SINGLETONS)];
        if (entity.components.WiredPins) {
            items.unshift(BOOL_FALSE_SINGLETON, BOOL_TRUE_SINGLETON);
            items.push(this.root.shapeDefinitionMgr.getShapeItemFromShortKey(this.root.gameMode.getBlueprintShapeKey()));
        }
        else {
            // producer which can produce virtually anything
            const shapes: any = ["CuCuCuCu", "RuRuRuRu", "WuWuWuWu", "SuSuSuSu"];
            items.unshift(...shapes.reverse().map((key: any): any => this.root.shapeDefinitionMgr.getShapeItemFromShortKey(key)));
        }
        if (this.root.gameMode.hasHub()) {
            items.push(this.root.shapeDefinitionMgr.getShapeItemFromDefinition(this.root.hubGoals.currentGoal.definition));
        }
        if (this.root.hud.parts.pinnedShapes) {
            items.push(...this.root.hud.parts.pinnedShapes.pinnedShapes.map((key: any): any => this.root.shapeDefinitionMgr.getShapeItemFromShortKey(key)));
        }
        ...
        const closeHandler: any = (): any => {
            if (!this.root || !this.root.entityMgr) {
                // Game got stopped
                return;
            }
            const entityRef: any = this.root.entityMgr.findByUid(uid, false);
            if (!entityRef) {
                // outdated
                return;
            }
            const constantComp: any = entityRef.components.ConstantSignal;
            if (!constantComp) {
        ...
        dialog.valueChosen.add((): any => {
            dialog.closeRequested.dispatch();
            closeHandler();
        });
        // When cancelled, destroy the entity again
        if (deleteOnCancel) {
            dialog.buttonSignals.cancel.add((): any => {
                if (!this.root || !this.root.entityMgr) {
                    // Game got stopped
                    return;
                }
