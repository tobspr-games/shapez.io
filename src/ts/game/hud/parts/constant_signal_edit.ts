...
export class HUDConstantSignalEdit extends BaseHUDPart {
    ...
    editConstantSignal(entity: Entity, { deleteOnCancel = true }: {
        ...
        if (entity.components.WiredPins) {
            ...
            items.unshift(...shapes.reverse().map(key => this.root.shapeDefinitionMgr.getShapeItemFromShortKey(key)));
        }
        ...
        if (this.root.hud.parts.pinnedShapes) {
            items.push(...this.root.hud.parts.pinnedShapes.pinnedShapes.map(key => this.root.shapeDefinitionMgr.getShapeItemFromShortKey(key)));
        }
