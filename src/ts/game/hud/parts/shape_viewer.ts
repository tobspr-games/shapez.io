    ...
    renderForShape(definition: ShapeDefinition) {
        ...
        for (let i = layers.length - 1; i >= 0; --i) {
            ...
            for (let quad = 0; quad < 4; ++quad) {
                ...
                if (contents) {
                    const colorLabelElem = makeDiv(quadElem, null, ["colorLabel"], T.ingame.colors[contents.color]);
                }
                else {
                    const emptyLabelElem = makeDiv(quadElem, null, ["emptyLabel"], T.ingame.shapeViewer.empty);
                }
            }
        }
    }
