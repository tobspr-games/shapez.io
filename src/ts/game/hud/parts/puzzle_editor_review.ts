...
export class HUDPuzzleEditorReview extends BaseHUDPart {
    ...
    startReview() {
        ...
        setTimeout(() => {
            ...
            const maxTicks = this.root.gameMode.getFixedTickrate() * globalConfig.puzzleValidationDurationSeconds;
    ...
    doSubmitPuzzle(title, shortKey) {
        ...
        this.root.app.clientApi
            ...
            .then(() => {
            closeLoading();
            const { ok } = this.root.hud.parts.dialogs.showInfo(T.dialogs.puzzleSubmitOk.title, T.dialogs.puzzleSubmitOk.desc);
            ok.add(() => this.root.gameState.moveToState("PuzzleMenuState"));
        }, err => {
            closeLoading();
            logger.warn("Failed to submit puzzle:", err);
            const signals = this.root.hud.parts.dialogs.showWarning(T.dialogs.puzzleSubmitError.title, T.dialogs.puzzleSubmitError.desc + " " + err, ["cancel", "retry:good"]);
